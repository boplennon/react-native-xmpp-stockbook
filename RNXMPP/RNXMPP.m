...
RCT_EXPORT_METHOD(querryLastArchivedMessages:(NSInteger)limit with:(NSString *)user before:(NSString *) before){
    [RNXMPPService sharedInstance].delegate = self;
     [[RNXMPPService sharedInstance] querryLastArchivedMessages:limit with:user before:before];
}
